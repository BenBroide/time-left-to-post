  ...
  Plugin.prototype = {
    ...
    updateTime: function () {
      ...
      var viewportHeight = $(window).height(),
       scrollbarHeight = viewportHeight / $("article").height() * viewportHeight,
       progress = $(window).scrollTop() / ($("article").height() - viewportHeight),
      ...
      var text = '';
      var thanks_text = tlb_data.thanks_text;
      var minutes_left_text = tlb_data.minutes_left_text;

      if(total_reading_time_remaining > 1) {
        text = total_reading_time_remaining + ' ' + minutes_left_text;
        $("#time-left").text(text);
      } else if(progress >= 1) {
        text = thanks_text;
        $("#time-left").text(thanks_text);
